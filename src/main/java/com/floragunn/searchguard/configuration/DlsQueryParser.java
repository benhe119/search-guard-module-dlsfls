...
final class DlsQueryParser {
  ...
  static Query parse(final Set<String> unparsedDlsQueries, final QueryShardContext queryShardContext, 
      ...
      for (final String unparsedDlsQuery : unparsedDlsQueries) {
          final XContentParser parser = XContentFactory.xContent(unparsedDlsQuery).createParser(namedXContentRegistry, unparsedDlsQuery);                
          final QueryBuilder qb = queryShardContext.newParseContext(parser).parseInnerQueryBuilder().get();
